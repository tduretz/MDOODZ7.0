...
void SetParticles( markers *particles, scale scaling, params model, mat_prop *materials  ) {    int np;
    ...
    for( np=0; np<particles->Nb_part; np++ ) {
        ...
        if ( (int)model.user0 == 1 ) {
            if (fabs(Z) < radius/2.0) {
        ...
        if ( model.eqn_state > 0 ) {
            particles->rho[np] = materials->rho[particles->phase[np]] * (1 -  materials->alp[particles->phase[np]] * (zeroC - materials->T0[particles->phase[np]]) );
        }
        else {
