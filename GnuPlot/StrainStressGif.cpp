...
static std::string path = "/home/roman/CLionProjects/MDOODZ7.0/cmake-exec/CollisionIra/heavy3";

void BuildStrainStress(set<fs::path> outputFiles) {
  ofstream myfile;
  ...
  for (fs::path filePath : outputFiles) {
    ...
    MatrixXf            eII  = (0.5 * (2 * exxdMatrix.array().square() + 0.5 * (exzMatrix.block<599, 399>(1, 1).array().square() + exzMatrix.block<599, 399>(0, 0).array().square() + exzMatrix.block<599, 399>(1, 0).array().square() + exzMatrix.block<599, 399>(0, 1).array().square()))).sqrt();
    ...
    MatrixXf            sII = (0.5 * (2 * sxxdMatrix.array().square() + 0.5 * (sxzMatrix.block<599, 399>(1, 1).array().square() + sxzMatrix.block<599, 399>(0, 0).array().square() + sxzMatrix.block<599, 399>(1, 0).array().square() + sxzMatrix.block<599, 399>(0, 1).array().square()))).sqrt();


    myfile << "\"{/:Italic t} = " << time / 1000000 << " [Myr]" << endl;
  ...
  int nFrames = outputFiles.size();

  myfile.close();
  char command[500];
  //snprintf(command, sizeof(command), R"(gnuplot -e "filename='../VISUAL_TESTS/img/StrainStressGif.gif'" -e "data='StrainStressGif.dat'" -e "frames=%d" StrainStressGIF.gnu)", nFrames);
  cout << command;
