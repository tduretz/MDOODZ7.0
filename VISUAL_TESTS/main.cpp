...
int SHD14SetPhase(MdoodzInstance *instance, Coordinates coordinates) {
  const double radius = instance->model.user1 / instance->scaling.L;
  if (coordinates.x * coordinates.x + coordinates.z * coordinates.z < radius * radius) {
    return 1;
  } else {
...
double SHD14SetTemperature(MdoodzInstance *instance, Coordinates coordinates) {
  return (instance->model.user0 + zeroC) / instance->scaling.T;
}

double SHD14SetDensity(MdoodzInstance *instance, Coordinates coordinates, int phase) {
  const double T_init = (instance->model.user0 + zeroC) / instance->scaling.T;
  if (instance->model.eqn_state > 0) {
    return instance->materials.rho[phase] * (1 - instance->materials.alp[phase] * (T_init - instance->materials.T0[phase]));
  } else {
...
SetBC SHD14SetBCVx(MdoodzInstance *instance, POSITION position, Coordinates coordinates) {
  SetBC bc;
  if (instance->model.shear_style == 0) {
    if (position == S || position == N || position == NE || position == NW || position == SE || position == SW) {
      bc.value = 0.0;
...
SetBC SHD14SetBCVz(MdoodzInstance *instance, POSITION position, Coordinates coordinates) {
  SetBC bc;
  if (instance->model.shear_style == 0) {
    if (position == W || position == E || position == NE || position == NW || position == SE || position == SW) {
      bc.value = 0.0;
...
SetBC SHD14SetBCT(MdoodzInstance *instance, POSITION position, double particleTemperature) {
  SetBC bc;
...
SetBC SHD14SetBCTNew(MdoodzInstance *instance, POSITION position, double particleTemperature) {
  SetBC bc;
  if (position == W || position == E || position == S || position == N || position == SE || position == SW || position == NE || position == NW) {
    bc.type  = 0;
    bc.value = 0.0;
...
MdoodzInstance CreateShearHeatingDuretz14Instance() {
  return (MdoodzInstance){
          .inputFileName = "ShearHeatingDuretz14.txt",
          .SetParticles  = new (SetParticles_ff){
...
void RunTestCases() {
  MdoodzInstance shearHeatingDuretz14 = CreateShearHeatingDuretz14Instance();
  RunMDOODZ(&shearHeatingDuretz14);
