...
void PlotShearTemplateAniso() {
  H5p::File file = H5p::File("ShearTemplateAniso.gzip.h5", "r");
  std::vector<double> params = file.read<std::vector<double>>("/Model/Params");
  const int nx = (int) params[3];
  const int nz = (int) params[4];
  ...
  std::ofstream myfile;
  myfile.open("ShearTemplateAniso.dat");
  int stepsToSkip = 9;
  int j_count = 0;
  int i_count = 0;
  for (int j = 0; j < nz - 1; j++) {
  ...
  std::FILE *GNUplotPipe = popen("gnuplot -e \"filename='../VISUAL_TESTS/img/ShearTemplateAniso.png'; data='ShearTemplateAniso.dat';\" ShearTemplate.gnu", "w");
  std::fflush(GNUplotPipe);
}

void PlotShearTemplateAnisoReference() {
  H5p::File file = H5p::File("ShearTemplateAnisoReference.h5", "r");
  std::vector<double> params = file.read<std::vector<double>>("/Model/Params");
  const int nx = (int) params[3];
  const int nz = (int) params[4];
  ...
  std::ofstream myfile;
  myfile.open("ShearTemplateAnisoReference.dat");
  int stepsToSkip = 9;
  int j_count = 0;
  int i_count = 0;
  for (int j = 0; j < nz - 1; j++) {
  ...
  std::FILE *GNUplotPipe = popen("gnuplot -e \"filename='../VISUAL_TESTS/img/ShearTemplateAnisoReference.png'; data='ShearTemplateAnisoReference.dat';\" ShearTemplate.gnu", "w");
  std::fflush(GNUplotPipe);
}
