...
double SetSurfaceZCoord(MdoodzInput *instance, double x_coord) {
  ...
  const double A  = 0*1e3 / instance->scaling.L;

  const double TopoLevel = A * sin(2*M_PI*x_coord / Lx * 16);
...
int SetPhase(MdoodzInput *instance, Coordinates coordinates) {
  ...
  const double lithosphereThickness = instance->model.user1 / Lc;
  const double weakZoneWidth        = 10e3/Lc;
  const double mohoLevel            = instance->model.user4 / Lc;
  ...
  // Set weak zone
  //  if (coordinates.z>-lithosphereThickness && coordinates.x>-weakZoneWidth + coordinates.z*2.9 && coordinates.x<weakZoneWidth + coordinates.z*2.9)
  ...
  //  // Draw ellipse
  // const double angle = 30.*M_PI/180;
  // const double a_ell =30e3/Lc, b_ell = 7e3/Lc;
  // double x_ell, z_ell;
  // const double x = coordinates.x, z = coordinates.z;
  // double X, Z;
  // Z = - 55e3/instance->scaling.L;
  // X = 20e3/instance->scaling.L;
  // x_ell = (x-X)*cos(angle) + (z-Z)*sin(angle);
  // z_ell =-(x-X)*sin(angle) + (z-Z)*cos(angle);
  // if (pow(x_ell/a_ell,2.0) + pow(z_ell/b_ell,2.0) < 1.0) phase = 2;
...
int main(int nargs, char *args[]) {
  ...
  char *input_file;
  if ( nargs < 2 ) {
    asprintf(&input_file, "MarsSubduction.txt"); // Default
  }
  else {
    printf("dodo %s\n", args[1]);
