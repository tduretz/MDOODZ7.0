...
int SetPhase(MdoodzInput *input, Coordinates coordinates) {
  ...
  const double Tcountry   = input->model.user0 / input->scaling.T;
  const double Tintrusion = input->model.user1 / input->scaling.T;
...
int main(int nargs, char *args[]) {
  ...
  char *input_file;
  if ( nargs < 2 ) {
    asprintf(&input_file, "CoolingChamber.txt"); // Default
  }
  else {
