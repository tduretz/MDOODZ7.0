...
int SetPhase(MdoodzInput *instance, Coordinates coordinates) {
  ...
  const bool   isContinentalPlate   = false; //coordinates.x > 0.0e3 / instance->scaling.L;
  const double x_ext_start          =  0.0e3 / instance->scaling.L;
  const double x_ext_end            =  100.0e3 / instance->scaling.L;
  const double marginWidth          =  x_ext_end - x_ext_start;
...
double SetTemperature(MdoodzInput *instance, Coordinates coordinates) {
  const double lithosphereThickness = instance->model.user1 / instance->scaling.L;
  const double Ts   = 273.15 / instance->scaling.T;
  const double Tm   = (1350.0 + 273.15) / instance->scaling.T;
  const double weakZoneWidth        =  10.0e3/instance->scaling.L;
  const double subductionAngle      = tan((90.0 - instance->model.user4)/180*PI);
  const double ocLithThickness      = instance->model.user5 / instance->scaling.L;
  ...
  const double yr  = 365.25*24*3600;
  const double age_left  = 45.0 * 1e6 * yr / instance->scaling.t;
  const double age_right = 55.0 * 1e6 * yr / instance->scaling.t;
  const double tau       = 50.0 * 1e6 * yr / instance->scaling.t;
  const double L         = instance->model.user5 / instance->scaling.L;
  const double D         = instance->model.user5 / instance->scaling.L;
  const double kappa                = 1.0e-6 / (instance->scaling.L*instance->scaling.L / instance->scaling.t);

  const double x1  = 100e3 / instance->scaling.L;
  const double z   = coordinates.z;
  const double age = coordinates.x<x1 ? age_left : age_right;


  double T = Ts;
  if (z<1e-3) T = Tm*erf(-z/(2*sqrt(kappa*age)));
  // if (z<1e-3) printf("%1.5e \n", age);
  // if (z>1e-3) T = Ts + (Tm - Ts) * (1 - sinh(z/L)/sinh(D/L)) + (Tm - Ts)*exp(-age/tau);
...
int main(int nargs, char *args[]) {
  ...
  char *input_file;
  if ( nargs < 2 ) {
    asprintf(&input_file, "ThermalStructureSubduction.txt"); // Default
  }
  else {
    printf("dodo %s\n", args[1]);
