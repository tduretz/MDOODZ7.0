...
int SetPhase(MdoodzInput *instance, Coordinates coordinates) {
  ...
  const double z0                   = -18e3 / instance->scaling.L;
  const double r                    = instance->model.user0 / instance->scaling.L;
  const bool   perturbation         = ((coordinates.x*coordinates.x) + (coordinates.z-z0)*(coordinates.z-z0)) < r*r;
  const double x1_bot               = -25e3 / instance->scaling.L;
  const double x2_bot               =  25e3 / instance->scaling.L;
  const double z1_bot               =  0.0 / instance->scaling.L;
  const double z2_bot               =  -crustThickness;
  const double a_bot                = (z2_bot -z1_bot )/(x2_bot-x1_bot);
  const double b_bot                = -crustThickness / 2; 

  // if (isAboveMoho) {
  //   if (coordinates.z < (coordinates.x*a_bot + b_bot)) 
  //     if ((coordinates.z+crustThickness) < ((coordinates.x-x1_bot)*a_bot + b_bot)) 
  //       return 2;
  //     else
  //       return 3;
  //   else
  ...
  // }
  int phase=3;
  if (coordinates.z > -crustThickness/2)  phase =2 ;
  if (perturbation) phase = 1; 
  // if (coordinates.z < (coordinates.x*a_bot + b_bot))  phase=2;
  // if (coordinates.z < ((coordinates.x+crustThickness)*a_bot + b_bot))  phase=3;
  // if (coordinates.x < x1_bot)  phase=2;
  // if (coordinates.x < x1_bot && coordinates.z<-crustThickness*0.75 )  phase=3;
  // if (coordinates.x > x2_bot && coordinates.z <-crustThickness )  phase=2;
  // if (coordinates.x > x2_bot && isBelowLithosphere )  phase=3;
  // if (coordinates.z < ((coordinates.x+crustThickness)*a_bot + b_bot) && coordinates.z < -crustThickness)  phase=3;
