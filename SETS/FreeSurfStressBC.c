...
double SetSurfaceZCoord(MdoodzInput *instance, double x_coord) {
  ...
  return TopoLevel + 0.05*x_coord;// + h_pert * (3330.0 - 2800.0) / 2800.0 * cos(2 * M_PI * x_coord / (instance->model.xmax - instance->model.xmin));
}
...
int main(int nargs, char *args[]) {
  ...
  char *input_file;
  if ( nargs < 2 ) {
    asprintf(&input_file, "FreeSurfStressBC.txt"); // Default
  }
  else {
