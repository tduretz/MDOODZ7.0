...
int SetPhase(MdoodzInput *instance, Coordinates coordinates) {
  ...
  const double z0                   = -45e3 / instance->scaling.L;
...
SetBC SetBCT(MdoodzInput *instance, POSITION position, Coordinates coordinates,  double particleTemperature) {
  ...
  double moho_temperature  = (500. + zeroC) / instance->scaling.T;
  if (position == S) {
...
int main(int nargs, char *args[]) {
  ...
  char *input_file;
  if (nargs < 2) {
    asprintf(&input_file, "CrustalShearBandingAnisotropy.txt");// Default
  } else {
    printf("dodo %s\n", args[1]);
