...
//  if (phase == 0) {
//    return 90;           // Layered aniso in crust
//  }
//  else {
//    return rand()*360;  // random value in mantle
//  }
//}
...
double SetAnisoAngle(MdoodzInput *input, Coordinates coordinates, int phase) {
  ...
  // (for 4 setup)
  double Xshift[4]  = { -1.5e5    , 0.5e5     , 1.3e5     , 0.0     };
  double Zshift[4]  = { 6.345e6   , 6.350e6   , 6.345e6   , 6.0e6   };
  double Majaxis[4] = { 0.7e5/4   , 0.6e5/4   , 0.9e5/4   , 1.0e5/2 };
  double Minaxis[4] = { 0.4e5/4   , 0.5e5/4   , 0.6e5/4   , 1.0e5/2 };
  double Angle[4]   = { 30        , 120       , 0         , 0.0     }; // direction of layers
  ...
  for (int i = 0; i < nellipses; ++i) {
    ...
    Angle[i]  = Angle[i] + 90; // have layers -> want director
    isInsideThisEllipse = ((coordinates.x*input->scaling.L - Xshift[i])*cos(Angle[i]) + (coordinates.z*input->scaling.L - Zshift[i])*sin(Angle[i]))*((coordinates.x*input->scaling.L - Xshift[i])*cos(Angle[i]) + (coordinates.z*input->scaling.L - Zshift[i])*sin(Angle[i]))/(Majaxis[i] * Majaxis[i]) + (-(coordinates.z*input->scaling.L - Zshift[i])*cos(Angle[i]) + (coordinates.x*input->scaling.L - Xshift[i])*sin(Angle[i]))*(-(coordinates.z*input->scaling.L - Zshift[i])*cos(Angle[i]) + (coordinates.x*input->scaling.L - Xshift[i])*sin(Angle[i]))/ (Minaxis[i] * Minaxis[i]) < 1; // rotated ellipse
    // reminder ellipse function: inside = [(x-x0)*cos(phi)+(z-z0)*sin(phi)]^2/majaxis^2 + [(x-x0)*sin(phi)-(z-z0)*cos(phi)]^2/minaxis^2 < 1
    if (isInsideThisEllipse && phase == 0) {
      isInsideSomeEllipse = true;
      return Angle[i]; // corresponding aniso_angle (i.e. director)
    }
  }
...
int main(int nargs, char *args[]) {
  ...
  char *input_file;
  if ( nargs < 2 ) {
    asprintf(&input_file, "CollisionPolarCartesianEllipses.txt"); // Default
  }
  else {
