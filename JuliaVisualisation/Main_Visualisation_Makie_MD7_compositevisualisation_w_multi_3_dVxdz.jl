...
function main(path, file_start, file_end_max,fac, xshift, file_step)
    ...
    fabricAlign = true      # also a coordinate transformation, transforms to fabric coordinates
    ...
    for istep=[file_end;file_start:file_step:file_end]    
        ...
        # Postprocessing deformation rate tensor
        if field==:dVxdz || field==:dVzdx || field==:dVzdx_percent
            dVxdx = diff(Vx, dims=1)./Δx
            dVzdz = diff(Vz, dims=2)./Δz
            dVxdz = diff(Vx, dims=2)./Δz
            dVzdx = diff(Vz, dims=1)./Δx

            #@show size(dVxdx),size(dVzdz),size(dVxdz),size(dVzdx)
        ...
        if polar
            ...
            ALPHA = asin.( XX ./ ( XX.^2 .+ ZZ.^2 ).^0.5 ) # angle of coordinate transformation
            τxx, τzz, τxz, τxz          = coordTransformTensor( τxx, τzz, τxz, τxz, ALPHA )
            ε̇xx, ε̇zz, ε̇xz, ε̇zx          = coordTransformTensor( ε̇xx,-ε̇xx, ε̇xz, ε̇xz, ALPHA )
            Vxc, Vzc                    = coordTransformVector( Vxc, Vzc, ALPHA )
            if field==:dVxdz || field==:dVzdx || field==:dVzdx_percent
            dVxdx, dVzdz, dVxdz, dVzdx  = coordTransformTensor( dVxdx, dVzdz, dVxdz, dVzdx, ALPHA ) end
        end
        # Postprocessing fabricAlign setups
        if fabricAlign
            ALPHA = asin.(Fab_z ./ ( Fab_x.^2 .+ Fab_z.^2 ).^0.5)
            τxx, τzz, τxz, τxz          = coordTransformTensor( τxx, τzz, τxz, τxz, ALPHA )
            ε̇xx, ε̇zz, ε̇xz, ε̇zx          = coordTransformTensor( ε̇xx,-ε̇xx, ε̇xz, ε̇xz, ALPHA )
            Vxc, Vzc                    = coordTransformVector( Vxc, Vzc, ALPHA )
            if field==:dVxdz || field==:dVzdx || field==:dVzdx_percent
        ...
        if field==:Stress_xz
            ...
            if polar || fabricAlign 
            hm = MoC.heatmap!(ax1, xc./Lc, zc./Lc, τxz, colormap = (:turbo, α_heatmap)) else
            hm = MoC.heatmap!(ax1, xv./Lc, zv./Lc, τxz, colormap = (:turbo, α_heatmap), colorrange=(0, 1e1)) end
        ...
        if field==:StrainRate_xz
            ax1 = MoC.Axis(f[1, 1], title = L"$\dot{\varepsilon}_\textrm{xz}$ at $t$ = %$(tMy) %$(tMytext)", xlabel = L"$x$ [m]", ylabel = L"$y$ [m]")
            if polar || fabricAlign
            hm = MoC.heatmap!(ax1, xc./Lc, zc./Lc, ε̇xz, colormap = (:turbo, α_heatmap), colorrange=(-3, 1.5)) else
            hm = MoC.heatmap!(ax1, xv./Lc, zv./Lc, ε̇xz, colormap = (:turbo, α_heatmap), colorrange=(-3, 1.5)) end
            MoC.Colorbar(f[1, 2], hm, label =  L"$\dot{\varepsilon}_\textrm{xz}$ [s$^{-1}$]", width = 20, labelsize = 25, ticklabelsize = 14 )
        end

        if field==:dVxdz
            ax1 = MoC.Axis(f[1, 1], title = L"$\textrm{dVxdz}$ at $t$ = %$(tMy) %$(tMytext)", xlabel = L"$x$ [m]", ylabel = L"$y$ [m]")
            if polar || fabricAlign
            hm = MoC.heatmap!(ax1, xc./Lc, zc./Lc, log10.(abs.(dVxdz)), colormap = (:turbo, α_heatmap), colorrange=(-3, 1.5)) else
            hm = MoC.heatmap!(ax1, xv./Lc, zv./Lc, log10.(abs.(dVxdz)), colormap = (:turbo, α_heatmap), colorrange=(-3, 1.5)) end
            MoC.Colorbar(f[1, 2], hm, label =  L"$\textrm{dVxdz}$ [s$^{-1}$]", width = 20, labelsize = 25, ticklabelsize = 14 )
        end

        if field==:dVzdx
            ax1 = MoC.Axis(f[1, 1], title = L"$\textrm{dVzdx}$ at $t$ = %$(tMy) %$(tMytext)", xlabel = L"$x$ [m]", ylabel = L"$y$ [m]")
            if polar || fabricAlign
            hm = MoC.heatmap!(ax1, xc./Lc, zc./Lc, log10.(abs.(dVzdx)), colormap = (:turbo, α_heatmap), colorrange=(-3, 1.5)) else
            hm = MoC.heatmap!(ax1, xv./Lc, zv./Lc, log10.(abs.(dVzdx)), colormap = (:turbo, α_heatmap), colorrange=(-3, 1.5)) end
            MoC.Colorbar(f[1, 2], hm, label =  L"$\textrm{dVzdx}$ [s$^{-1}$]", width = 20, labelsize = 25, ticklabelsize = 14 )
        end

        if field==:dVzdx_percent
            ax1 = MoC.Axis(f[1, 1], title = L"$\textrm{dVzdx / (dVxdz+dVzdx) * 100%}$ at $t$ = %$(tMy) %$(tMytext)", xlabel = L"$x$ [m]", ylabel = L"$y$ [m]")
            if polar || fabricAlign
            hm = MoC.heatmap!(ax1, xc./Lc, zc./Lc, dVzdx ./ dVxdz * 100 , colormap = (:turbo, α_heatmap), colorrange=(0, 100)) else
            hm = MoC.heatmap!(ax1, xv./Lc, zv./Lc, dVzdx ./ dVxdz * 100 , colormap = (:turbo, α_heatmap), colorrange=(0, 100)) end
            MoC.Colorbar(f[1, 2], hm, label =  L"$\textrm{dVzdx / (dVxdz+dVzdx) * 100%}$ [s$^{-1}$]", width = 20, labelsize = 25, ticklabelsize = 14 )
        end
        ...
        if field==:Velocity_x
            ...
            if polar || fabricAlign
            hm = MoC.heatmap!(ax1, xc./Lc, zc./Lc, Vxc           , colormap = (:turbo, α_heatmap), colorrange=(-0.2, 0.2)) else 
            hm = MoC.heatmap!(ax1, xv./Lc, zc./Lc, Vx[:,2:end-1] , colormap = (:turbo, α_heatmap), colorrange=(-0.2, 0.2)) end
        ...
        if field==:Velocity_z
            ...
            if polar || fabricAlign
            hm = MoC.heatmap!(ax1, xc./Lc, zc./Lc, Vzc           , colormap = (:turbo, α_heatmap), colorrange=(-0.2, 0.2)) else 
            hm = MoC.heatmap!(ax1, xc./Lc, zv./Lc, Vz[2:end-1,:] , colormap = (:turbo, α_heatmap), colorrange=(-0.2, 0.2)) end
...
function coordTransformTensor( AXX, AZZ, AXZ, AZX, ALPHA)
...
function coordTransformVector( Ax, Az, ALPHA )

    Ar =  Ax.*cos.(ALPHA) + Az.*sin.(ALPHA)
    Ap = -Ax.*sin.(ALPHA) + Az.*cos.(ALPHA)
...
## all aniso setups
#main("/users/whalter1/work/aniso_fix/A2_1000/", 2790, 12000, 4, 0.5, file_step)
#main("/users/whalter1/work/aniso_fix/A3_1000/", 3320, 12000, 4, 0.5, file_step)
main("/users/whalter1/work/aniso_fix/A4_1000/", 2770, 12000, 4, 0.5, file_step)
#main("/users/whalter1/work/aniso_fix/A5_1000/", 3460, 12000, 4, 0.5, file_step)
#main("/users/whalter1/work/aniso_fix/MWE_1000/", 400, 12000, 4, 0.5, file_step)
#main("/users/whalter1/work/aniso_fix/MWE_500/" , 400, 12000, 2, 0.5, file_step)

if aniSetupsOnly == false
    ## all iso setups
    #main("/users/whalter1/work/aniso_fix/A1_1000/", 2610, 12000, 4, 0.5, file_step)
    #main("/users/whalter1/work/aniso_fix/A7_1000/", 3460, 12000, 4, 0.5, file_step)
    ##main("/users/whalter1/work/aniso_fix/A7_500/" , 5000, 12000, 2, 0.5, file_step)
    #main("/users/whalter1/work/aniso_fix/A6_1000/", 3460, 12000, 4, 0.5, file_step)
    #main("/users/whalter1/work/aniso_fix/B1_1000/", 0, 12000, 4, 0.5, file_step)
    #main("/users/whalter1/work/aniso_fix/B2_1500/", 0, 12000, 6, 0.5, file_step)
    #main("/users/whalter1/work/aniso_fix/B2_1000/", 0, 12000, 4, 0.5, file_step)
    #main("/users/whalter1/work/aniso_fix/B3_1500/", 0, 12000, 6, 0.5, file_step)
    #main("/users/whalter1/work/aniso_fix/B3_1250/", 0, 12000, 5, 0.5, file_step)
    #main("/users/whalter1/work/aniso_fix/B3_1000/", 0, 12000, 4, 0.5, file_step)
    #main("/users/whalter1/work/aniso_fix/B3_750/" , 0, 12000, 3, 0.5, file_step)
    #main("/users/whalter1/work/aniso_fix/B4_1500/", 0, 12000, 6, 0.5, file_step)
    #main("/users/whalter1/work/aniso_fix/B5_1000/", 0, 12000, 4, 0.5, file_step)
end
