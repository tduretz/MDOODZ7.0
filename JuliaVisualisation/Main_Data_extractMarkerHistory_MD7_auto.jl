...
function main()
    ...
    simList = ["MODEL1_1x1k_Serp_25Phi_1Prefpwl_NewCode_6kmSerp_WesterlyGranite_ReducedConvRate10mmyr_ParticlesCorr/"]
    # Switches
    ...
    for iSim in eachindex(simList)
        path = string(simDir, simList[iSim])
        
        pFileList = filter(contains("Particles"), readdir(path))
        ...
        # Load particle data
        fmark_first = string(path, pFileList[1  ])
        fmark_last  = string(path, pFileList[end])
        println(fmark_first)
        println(fmark_last)
        ...
        if writeHistory
            vxm_time   = zeros(Float64, size(target_ind,1), size(pFileList,1))
            vzm_time   = zeros(Float64, size(target_ind,1), size(pFileList,1))
            xm_time    = zeros(Float64, size(target_ind,1), size(pFileList,1))
            zm_time    = zeros(Float64, size(target_ind,1), size(pFileList,1))
            Tm_time    = zeros(Float64, size(target_ind,1), size(pFileList,1))
            Pm_time    = zeros(Float64, size(target_ind,1), size(pFileList,1))
            phm_time   = zeros(Float64, size(target_ind,1), size(pFileList,1))
            sxxdm_time = zeros(Float64, size(target_ind,1), size(pFileList,1))
            sxzm_time  = zeros(Float64, size(target_ind,1), size(pFileList,1))
            tm_time    = zeros(Float64, 1,                  size(pFileList,1))
        end
