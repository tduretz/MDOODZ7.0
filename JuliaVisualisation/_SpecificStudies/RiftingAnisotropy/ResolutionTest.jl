...
@views function main()
    ...
    options = (
        ...
        printvid    = false,
        framerate   = 6,
        α_heatmap   = 0.99,   # transparency of heatmap 
        vel_arrow   = 5,
        vel_scale   = 10000,
        vel_step    = 20,
    ...
    if field==:StrainRate
        ...
        hm = heatmap!(ax1, xc./Lc, zc./Lc, log10.(ε̇II.ε̇II), colormap = (cmap, options.α_heatmap), colorrange=(-17, -13))
        # AddCountourQuivers!(PlotOnTop, ax1, coords, V, T, ϕ, σ1, ε̇1, Fab, height, Lc, cm_y, all_phases.group_phases, Δ/2)  
        ...
        hm = heatmap!(ax1, xc./Lc, zc./Lc, log10.(ε̇II.ε̇II), colormap = (cmap, options.α_heatmap), colorrange=(-17, -13))
        # AddCountourQuivers!(PlotOnTop, ax1, coords, V, T, ϕ, σ1, ε̇1, Fab, height, Lc, cm_y, all_phases.group_phases, Δ, Mak)    
        AddCountourQuivers!(PlotOnTop, ax1, coords, V, T, ϕ, σ1, ε̇1, PT, Fab, height, Lc, cm_y, all_phases.group_phases, 1.5Δ, Mak)                            
        colsize!(f.layout, 1, Aspect(1, Lx/Lz))
