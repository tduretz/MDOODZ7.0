...
@views function main()
    ...
    path  = [
    "/Users/tduretz/REPO/MDOODZ7.0/RUNS/RiftingAnisotropy/v_slow/",
    "/Users/tduretz/REPO/MDOODZ7.0/RUNS/RiftingAnisotropy/v_fast/",
    ...
    if field==:StrainRate
        ...
        ax1 = Axis(f[1, 1], title = L"A) $V_\textrm{ext} = $0.785 cm/y - 25% extension ($t$ = %$(tMy_string) Ma)", ylabel = L"$y$ [%$(length_unit)]", xgridvisible = false, ygridvisible = false,)
        ...
        tMy_string = @sprintf("%1.2lf", tMy)
        ax1 = Axis(f[3, 1], title = L"C) $V_\textrm{ext} = $3.14 cm/y - 25% extension ($t$ = %$(tMy_string) Ma)", ylabel = L"$y$ [%$(length_unit)]", xgridvisible = false, ygridvisible = false,)
        ...
        tMy_string = @sprintf("%1.2lf", tMy)
        ax1 = Axis(f[2, 1], title = L"B) $V_\textrm{ext} = $1.570 cm/y - 25% extension ($t$ = %$(tMy_string) Ma)", ylabel = L"$y$ [%$(length_unit)]", xgridvisible = false, ygridvisible = false,)
        # hm = heatmap!(ax1, xc./Lc, zc./Lc, log10.(τII), colormap = (cmap, options.α_heatmap), colorrange=(6.69, 8.69))
        hm = heatmap!(ax1, xc./Lc, zc./Lc, log10.(ε̇II.ε̇II), colormap = (cmap, options.α_heatmap), colorrange=(-16.5, -13.25))
        AddCountourQuivers!(PlotOnTop, ax1, coords, V, T, ϕ, σ1, ε̇1, PT, Fab, height, Lc, cm_y, all_phases.group_phases, 1.5Δ, Mak)                
        colsize!(f.layout, 1, Aspect(1, Lx/Lz))
        ...
        save("/Users/tduretz/PowerFolders/_manuscripts/RiftingAnisotropy/Figures/CompareVelocity.png", f, px_per_unit = 4)   
    end
