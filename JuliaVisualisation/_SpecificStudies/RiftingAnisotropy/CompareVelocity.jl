...
@views function main()
    ...
    f = Figure(size = (1.2*Lx/4/Lz*options.resol*1.2, options.resol), fontsize=ftsz)
    ...
    if field==:StrainRate
        ...
        ax1 = Axis(f[1, 1], title = L"A) $V_\textrm{ext} = $0.75 cm/y - 25% extension ($t$ = %$(tMy_string) Ma)", ylabel = L"$y$ [%$(length_unit)]", xgridvisible = false, ygridvisible = false,)
        ...
        tMy_string = @sprintf("%1.2lf", tMy)
        ax1 = Axis(f[2, 1], title = L"B) $V_\textrm{ext} = $3.00 cm/y - 25% extension ($t$ = %$(tMy_string) Ma)", ylabel = L"$y$ [%$(length_unit)]", xgridvisible = false, ygridvisible = false,)
        hm = heatmap!(ax1, xc./Lc, zc./Lc, log10.(τII), colormap = (cmap, options.α_heatmap), colorrange=(6.69, 8.69))
        # hm = heatmap!(ax1, xc./Lc, zc./Lc, log10.(ε̇II), colormap = (cmap, options.α_heatmap), colorrange=(-17, -13))
        AddCountourQuivers!(PlotOnTop, ax1, coords, V, T, ϕ, σ1, ε̇1, PT, Fab, height, Lc, cm_y, group_phases, 1.5Δ, Mak)                
        colsize!(f.layout, 1, Aspect(1, Lx/Lz))
