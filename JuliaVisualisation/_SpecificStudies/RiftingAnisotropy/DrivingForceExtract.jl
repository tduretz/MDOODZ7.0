...
@views function main()
    ...
    for istep=file_start:file_step:file_end
        ...
        if field==:DrivingForce
            ε̇vp = ε̇pl[isnan.(ε̇pwl).==false] .+ ε̇pwl[isnan.(ε̇pwl).==false] .+ ε̇exp[isnan.(ε̇pwl).==false] 
            Wvp = τII[isnan.(ε̇pwl).==false].*(ε̇vp) 
            # @show sum(Wvp)./sum(ε̇vp)*Δx*Δz, sum(τII[isnan.(ε̇pwl).==false])*Δx*Δz
            push!(F_Taras, sum(Wvp)./sum(ε̇vp)*Δx*Δz)
            τII_W =  τII[1, isnan.(τII[1,:]).==false]
            F_W = 2*sum(τII_W)*Δz
            τII_E =  τII[end, isnan.(τII[end,:]).==false]
            F_E = 2*sum(τII_E)*Δz
            push!(F_driving, 0.5*(F_W + F_E))
            if istep==file_end
                ax1 = Axis(f[1, 1], title = L"$$Driving force", xlabel = L"$t$ [Ma]", ylabel = L"$F$ [TN/m]")
                lines!(ax1, tvec, F_Taras  ./1e12, label="Taras" )
                lines!(ax1, tvec, F_driving./1e12, label="Driving force")
                axislegend()
                display(f)
                save(path*"DrivingForce.jld2", "time", tvec, "F_driving", F_driving, "F_Taras", F_Taras )
            end
        end
