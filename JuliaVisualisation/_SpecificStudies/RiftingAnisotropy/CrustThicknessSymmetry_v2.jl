...
@views function main()
    ...
    cm_yr = 100.0*3600.0*24.0*365.25
        ...
        if field==:Phases
            ...
            lines!(ax2, coords.c_hr.x./1e3, d1_thick.lith./1e3  ,   label=L"$\delta = 1$")  
            lines!(ax2, coords.c_hr.x./1e3, d4_thick.lith./1e3, label=L"$\delta = 4$ (ref)", color=:black, linestyle=:dash)  
            lines!(ax2, coords.c_hr.x./1e3, d5_thick.lith./1e3  ,   label=L"$\delta = 5$")  
            lines!(ax2, coords.c_hr.x./1e3, d10_thick.lith./1e3  ,   label=L"$\delta = 10$")  
