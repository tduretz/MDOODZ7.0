...
function main_simple_ani_vis()
    ...
    ax1 = Axis(f[3, 1], aspect=DataAspect(), title=L"$\theta = 0^\circ$")
   ...
   layering = Layering(
    ...
    ax1 = Axis(f[3, 5], aspect=DataAspect(), title=L"$\theta = 180^\circ$")
    heatmap!(ax1, xc, yc, phase, colormap=:grays)
    hidedecorations!(ax1)
    rowsize!(f.layout, 1, Relative(0.5))

    ax1 = Axis(f[1, 1:5], title=L"$$Horizontal extension: material & coordinate invariants", xlabel=L"$\theta$ [$^\circ$]", ylabel=L"$\tau_{II}$, $\tau_{II}'$  [-]")
    lines!(ax1, θ*180/π, τii_cart, label=L"$\tau_{II}$  ($\delta=2$, transformations)")
    scatter!(ax1, θ[1:3:end]*180/π, τii_cart_MD7[1:3:end], label=L"$\tau_{II}$  ($\delta=2$, tensor)")

    tii = 1.0*sqrt.(((δ^2-1)*cos.(2*θ).^2 .+ 1)/δ^2)
    scatter!(ax1, θ[1:2:end]*180/π, tii[1:2:end], marker=:dtriangle, label=L"$\tau_{II}$  ($\delta=2$, expression)" )
    ...
    f[2, 1:5] = Legend(f, ax1, framevisible = false, orientation = :horizontal, labelsize=15, nbanks=3)
