...
function Asymmetry( path, istep, show )
    ...
    f = Figure(size = (Lx/Lz*resol*1.2, resol), fontsize=40)
        ...
        if field==:Phases
            ...
            if show 
                ...
                ax1 = Axis(f[1, 1], title = L"$t$ = %$(tMy_string) Ma", xlabel = L"$x$ [km]", ylabel = L"$y$ [km]")
            ...
            return error_cr, error_lit, abs(𝐗_moho[1] - 𝐗_lab[1])
...
let 
    ...
    root_folder = "/Users/tduretz/REPO/MDOODZ7.0/RUNS/RiftingAnisotropy/systematics/"
    data = load(root_folder*"Asymmetry$(time)Ma.jld2")

    delta  = data["delta"]
    theta  = data["theta"]
    crust  = data["asym_crust"]
    mantle = data["asym_mantle"]
    Stefan = data["asym_Stefan"]
...
phase_colors = cgrad(cmap, length(cmap), categorical=true, rev=false)
    ...
    save("/Users/tduretz/PowerFolders/_manuscripts/PureSimpleShearAnisotropy/Figures/Systematics_proof.png", f, px_per_unit = 4)   
