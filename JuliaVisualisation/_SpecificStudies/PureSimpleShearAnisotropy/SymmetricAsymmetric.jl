...
@views function main()
    ...
    if field==:StrainRate
        ...
        lines!(𝐗_moho[1].*ones(size(zc)), zc./Lc, color=:white, linestyle=:dash, linewidth=3)
        lines!( 𝐗_lab[1].*ones(size(zc)), zc./Lc, color=:white, linestyle=:dash, linewidth=3)
        ...
        stretch = tMy * (1e-15*3600*24*365.25*1e6) *100
        stretch_string = @sprintf("%1.2lf", stretch)
        ax1 = Axis(f[3, 1], title = L"B) Anisotropic model ($\delta = 6.0$) - $\varepsilon$ = %$(stretch_string) %", ylabel = L"$y$ [%$(length_unit)]", xgridvisible = false, ygridvisible = false,)
        ...
        save("/Users/tduretz/PowerFolders/_manuscripts/PureSimpleShearAnisotropy/Figures/SymmetricAsymmetric_Phase.png", f, px_per_unit = 4)     end
