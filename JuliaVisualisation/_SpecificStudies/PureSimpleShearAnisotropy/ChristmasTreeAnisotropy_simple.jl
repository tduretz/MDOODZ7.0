...
@views function ChrismasTreeAniso()
    ...
    for i in eachindex(θ)
        ...
        # lines!(ax1, P./1e9, yc./1e3, color=:tomato, label=L"$T$"  )

        # lines!(ax1, P./1e9, yc./1e3, color=:blue, label=L"$P$" )
        ...
        xLetterDict = Dict{Float64,String}(0 => "0", 200 => "200", 400 => "400", 600 => "600")

        xLetteredValues = collect(keys(xLetterDict))

        ax3 = Axis(f[1, 1], title = L"B) $$Lithospheric stress", xlabel = L"$τ_\mathrm{II}$ [MPa]", ylabel = L"$y$ [km]", xticks = xLetteredValues)
        lines!(ax3, τii_rot1./1e6, yc./1e3, label=L"$\theta = 0^\text{o}$", color=:black )
        xlims!(ax3, 0, 700)
        # lines!(ax3, τii_rot2./1e9, yc./1e3 )
        lines!(ax3, τii_cart1./1e6, yc./1e3, label=L"$\theta = 45^\text{o}$", color=:gray )

        lines!(ax3, τii_yield./1e6,     yc./1e3, linestyle=:dash, color=:black )
        lines!(ax3, τii_yield_min./1e6, yc./1e3, linestyle=:dash, color=:gray )

        # lines!(ax3, τii_cart2./1e9, yc./1e3 )
        axislegend(position=:rb)
        display(f)

        save("/Users/tduretz/PowerFolders/_manuscripts/PureSimpleShearAnisotropy/Figures/ChristmasTreeSimple.png", f, px_per_unit = 4) 
