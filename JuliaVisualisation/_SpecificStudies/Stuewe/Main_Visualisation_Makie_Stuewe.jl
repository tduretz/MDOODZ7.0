...
function AddCountourQuivers!(PlotOnTop, ax1, xc, zc, V, T, σ1, Fab, Lc, cm_y, group_phases, Δ, Mak)
...
function main()
    ...
    for istep=file_start:file_step:file_end
        ...
        centx = Int64(floor(ncx/2))
        centz = Int64(floor(ncx/2))
        push!(T_center, T[centx, centz])
        push!(t_center, t)
        ...
        if field==:Phases
            ...
            AddCountourQuivers!(PlotOnTop, ax1, xc, zc, V, T, σ1, Fab, Lc, cm_y, group_phases, Δ, Mak)     
        ...
        if field==:TemperatureEvolution
            ax1 = Axis(f[1, 1], title = L"$T$ @ center [C]", xlabel = L"$t$ [My]", ylabel = L"$T$ @ center [C]")
            lines!(ax1, Float64.(t_center./My), Float64.(T_center))
            ylims!(ax1, 400, 1000)
