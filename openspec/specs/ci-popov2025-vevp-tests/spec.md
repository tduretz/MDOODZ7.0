...
Assertions combine final-step yield-surface membership with full-trajectory L2 against the standalone reference integrator. MDOODZ writes the globally-discretised pressure `p*` to HDF5 (`/Centers/P`). Empirically, the global Stokes solve converges with `p*` already on the yield surface for cap-arc and DP-envelope states (Mixed, Shear). At the **cap apex** (vertical tangent, where Volumetric Extension saturates), the discretisation produces a `K·θ̇_vp·dt` residual; the yield-clamped local pressure there is reconstructed via paper Eq. 31: `p_local = p* + K·θ̇_vp·dt` using `/Centers/divu_pl` for the apex membership and L2(P) checks. Volumetric Extension uses `out_of_plane = 1` so the constitutive law sets `ε̇_yy = 0.5·(ε̇_xx + ε̇_zz)` per cell — paper's true 3D 0D loading, deviator identically zero.

#### Scenario: Volumetric extension saturates at the tensile cap apex
- **WHEN** the test runs `Popov2025/Popov0D_VolumetricExtension.txt` with `out_of_plane = 1`, imposed strain rates `ε̇_xx = ε̇_zz = 2.333·10⁻¹⁵ s⁻¹` (outward normal BCs `user2 = 1.167·10⁻¹⁵ m/s`, `user3 = 8.167·10⁻¹⁶ m/s` — scaled by `Lz/Lx = 0.7`), `plast = 2`, `η^vp = 0`
- **THEN** the centre-cell `/Centers/divu_pl` SHALL be strictly positive (dilatant flow active on the cap), the reconstructed `sII` SHALL be at most 0.1 % of `τ_d` (the constitutive deviator vanishes by construction), and the apex point `(sII, p_local)` with `p_local = p* + K·dt·divu_pl` SHALL lie within 5 % of the tensile cap circle radius (`|R̂_y − R_y| / R_y < 0.05`). Full-trajectory L2 bounds: `RMS(sII) / τ_d < 0.5 %` and `L2(p_local) < 5 %` against the standalone reference
...
- **THEN** a PNG SHALL land at `TESTS/Popov2025/plots/out/fig5_0d_stress.png` showing the four panels `(a) volumetric extension`, `(b) deviatoric shear`, `(c) mixed strain`, `(d) meridional P–τ_II trajectories`. Panels (a)–(c) SHALL plot raw `/Centers/P` over time (column 4). Panel (d) SHALL plot the pressure variable that lands each trajectory on the yield surface: **Volumetric Extension** uses the reconstructed `p_local = p* + K·θ̇_vp·dt` (column 7 of `trajectory.dat`, computed by `extract_popov2025` from `/Centers/P` and `/Centers/divu_pl`) so it lands at the cap apex; **Shear** and **Mixed** use `/Centers/P` directly (column 4) — they sit on the DP envelope and cap arc respectively without correction
