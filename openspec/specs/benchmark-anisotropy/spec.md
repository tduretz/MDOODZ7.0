...
- **WHEN** a homogeneous simple-shear simulation runs for 40 time steps (dt=0.0125) with constant anisotropy factor δ=2, initial director at 45°, and simple shear rate γ̇=1.0 (bkg_strain_rate=0.5, shear_style=1)
- **THEN** the L2 error between the numerical director field and the analytically integrated director SHALL be less than 5e-3 radians
