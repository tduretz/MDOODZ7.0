...
- [x] 8.1 Push branch, start EC2 instance, pull and build on EC2
- [x] 8.2 Run benchmark sweep: `--scenario RiftingComprehensive --resolutions "highres" --threads "1 2 4 6 8 12 16" --steps 10 --sed 's/^thermal_solver.*/thermal_solver = 1/' --sed 's/^interp_mode.*/interp_mode = 3/'`
- [x] 8.3 Download results, verify `avg_interp_s < 1.5s` at 16 threads and no >10% regression at 1 thread vs mode 2 baseline
- [x] 8.4 Document results as Experiment 7 in `skill-benchmarking/SKILL.md` with hypothesis, implementation summary, results table, and conclusion
