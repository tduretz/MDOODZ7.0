...
- [x] 4.3 Implement `--validate` mode: run 3 steps per resolution (low→high), 1 thread, bail on first failure
...
- [x] 5.2 Run preflight — if it fails, stop and guide user through setup
- [x] 5.3 Verify: AWS CLI configured, EC2 instance accessible, SSH works, S3 bucket writable

## 6. AWS Automation Scripts

- [x] 6.1 Create `misc/benchmark-aws.sh`: preflight → start instance → wait SSH → rsync repo → remote exec → download summary → stop instance
...
- [x] 6.8 ~~Test locally: run `benchmark-ec2-setup.sh` in WSL~~ — skipped (no WSL; validated on EC2 directly)
...
- [x] 7.3 Update `.github/copilot-instructions.md` skill table if needed — already has benchmarking entry
