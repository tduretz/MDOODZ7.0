...
#pragma omp parallel for shared( mesh )  private ( ani, d1, d2 ) firstprivate ( model )
  ...
  if ( Jacobian==1 ) {
    ...
    // Loop on cell centers
#pragma omp parallel for shared( mesh ) private ( ani, d1, d2, angle, nx2, nxnz, eta_e, K, Da11, Da12, Da13, Da22, Da23, Da33, iDa11, iDa12, iDa13, iDa22, iDa23, iDa33, a11, a12, a13, a22, a23, a33, det, Exx, Ezz, Exz, gxz, Gxx, Gzz, Gxz ) firstprivate ( model, dt, comp )
    for (k=0; k<Ncx*Ncz; k++) {
      ...
      if ( mesh->BCp.type[k] != 30 && mesh->BCp.type[k] != 31 ) {
        ...
        //----------------------------------------------------------//
        if ( model->aniso == 0 ) {
          ani = 0.0; d1   = 0.0; d2   = 0.0; angle = 0.; nxnz = 0.; nx2 = 0.;
        }
        else {
          // Anisotropy
        ...
        //----------------------------------------------------------//
        mesh->D11_n[k] = 2.0*(1.0 - ani*d1)*eta_vep + 2.0*detadexx*Dxx - K*dt*ddivpdexx + 2.0*danidexx*Axx;
        mesh->D12_n[k] =         2.0*ani*d1*eta_vep + 2.0*detadezz*Dxx - K*dt*ddivpdezz + 2.0*danidezz*Axx;
        mesh->D13_n[k] =       - 2.0*ani*d2*eta_vep + 2.0*detadgxz*Dxx - K*dt*ddivpdgxz + 2.0*danidgxz*Axx;
        mesh->D14_n[k] =                              2.0*detadp  *Dxx - K*dt*ddivpdp   + 2.0*danidp  *Axx;
        //----------------------------------------------------------//
        mesh->D21_n[k] =         2.0*ani*d1*eta_vep + 2.0*detadexx*Dzz - K*dt*ddivpdexx + 2.0*danidexx*Azz;
        mesh->D22_n[k] = 2.0*(1.0 - ani*d1)*eta_vep + 2.0*detadezz*Dzz - K*dt*ddivpdezz + 2.0*danidezz*Azz;
        mesh->D23_n[k] =         2.0*ani*d2*eta_vep + 2.0*detadgxz*Dzz - K*dt*ddivpdgxz + 2.0*danidgxz*Azz;
    ...
    // Loop on cell vertices
#pragma omp parallel for shared( mesh )  private ( ani, d1, d2, angle, nx2, nxnz, eta_e, Da11, Da12, Da13, Da22, Da23, Da33, iDa11, iDa12, iDa13, iDa22, iDa23, iDa33, a11, a12, a13, a22, a23, a33, det, Exx, Ezz, Exz, gxz, Gxx, Gzz, Gxz  ) firstprivate ( model)
    for (k=0; k<Nx*Nz; k++) {
