...
void PhaseRheologyLoop_v1( int is_centroid, double sign, double denom, double Exx, double Ezz, double Exz, double P, double ani_fstrain, double ani_fac_e, double d0, double d1, double angle, double nx2, double nxnz, int c, double** vol,
...
void DerivativesOnTheFly_n( double* detadexx, double* detadezz, double* detadgxz, double* detadp, double* ddivpdexx, double* ddivpdezz, double* ddivpdgxz, double* ddivpdp, double* danidexx, double* danidezz, double* danidgxz, double* danidp, double* drhodp, int c0, double Exx_ref, double Ezz_ref, double Exz_ref, double P_ref, double ani, double ani_e, double d1, double d2, double angle, double nx2, double nxnz, grid *mesh, mat_prop *materials, params *model, scale *scaling ) {
    ...
    PhaseRheologyLoop_v1(  1, 1.0, 2.0*pert_xx, Exx_ref+pert_xx, Ezz_ref, Exz_ref, P_ref, ani, ani_e, d1, d2, angle, nx2, nxnz, c0, mesh->phase_perc_n,
    ...
    PhaseRheologyLoop_v1( 1, 1.0, 2.0*pert_zz, Exx_ref, Ezz_ref+pert_zz, Exz_ref, P_ref, ani, ani_e, d1, d2, angle, nx2, nxnz, c0, mesh->phase_perc_n,
                        mesh->mu_n, mesh->T, mesh->p0_n, mesh->T0_n[c0],
                        mesh->d0_n, mesh->phi0_n, mesh->X0_n,
                        mesh->sxxd0, mesh->szzd0, mesh->sxz0_n,
                        mesh->bet_n, mesh->div_u,
    ...
    PhaseRheologyLoop_v1(  1, 1.0, 4.0*pert_xz, Exx_ref, Ezz_ref, Exz_ref+pert_xz, P_ref, ani, ani_e, d1, d2, angle, nx2, nxnz, c0, mesh->phase_perc_n,
                        mesh->mu_n, mesh->T, mesh->p0_n, mesh->T0_n[c0],
                        mesh->d0_n, mesh->phi0_n, mesh->X0_n,
                        mesh->sxxd0, mesh->szzd0, mesh->sxz0_n,
                        mesh->bet_n, mesh->div_u,
    ...
    PhaseRheologyLoop_v1(  2, 1.0, 2.0*pert_p, Exx_ref, Ezz_ref, Exz_ref, P_ref+pert_p, ani, ani_e, d1, d2, angle, nx2, nxnz, c0, mesh->phase_perc_n,
                        mesh->mu_n, mesh->T, mesh->p0_n, mesh->T0_n[c0],
                        mesh->d0_n, mesh->phi0_n, mesh->X0_n,
                        mesh->sxxd0, mesh->szzd0, mesh->sxz0_n,
                        mesh->bet_n, mesh->div_u,
...
void DerivativesOnTheFly_s( double* detadexx, double* detadezz, double* detadgxz, double* detadp, double* danidexx, double* danidezz, double* danidgxz, double* danidp, int c1, double Exx_ref, double Ezz_ref, double Exz_ref, double P_ref, double ani, double ani_e, double d1, double d2, double angle, double nx2, double nxnz, grid *mesh, mat_prop *materials, params *model, scale *scaling ) {
    ...
    // 1) Positive perturbation in Exx
    PhaseRheologyLoop_v1(  0, 1.0, 2.0*pert_xx, Exx_ref+pert_xx, Ezz_ref, Exz_ref, P_ref, ani, ani_e, d1, d2, angle, nx2, nxnz, c1, mesh->phase_perc_s,
    ...
    // 1) Positive perturbation in Ezz
    PhaseRheologyLoop_v1(  0, 1.0, 2.0*pert_zz, Exx_ref, Ezz_ref+pert_zz, Exz_ref, P_ref, ani, ani_e, d1, d2, angle, nx2, nxnz, c1, mesh->phase_perc_s,
                        mesh->mu_s, mesh->T_s, mesh->p0_s, 0.0,
                        mesh->d0_s, mesh->phi0_s, mesh->X0_s,
                        mesh->sxxd0_s, mesh->szzd0_s, mesh->sxz0,
                        mesh->bet_s, mesh->div_u_s,
    ...
    PhaseRheologyLoop_v1(  0, 1.0, 4.0*pert_xz, Exx_ref, Ezz_ref, Exz_ref+pert_xz, P_ref, ani, ani_e, d1, d2, angle, nx2, nxnz, c1, mesh->phase_perc_s,
                        mesh->mu_s, mesh->T_s, mesh->p0_s, 0.0,
                        mesh->d0_s, mesh->phi0_s, mesh->X0_s,
                        mesh->sxxd0_s, mesh->szzd0_s, mesh->sxz0,
                        mesh->bet_s, mesh->div_u_s,
    ...
    //----------------------------------------------------------------------------------------------------------------------------------------------------//
    // 1) Positive perturbation in P
    PhaseRheologyLoop_v1(  0, 1.0, 2.0*pert_p, Exx_ref, Ezz_ref, Exz_ref, P_ref+pert_p, ani, ani_e, d1, d2, angle, nx2, nxnz, c1, mesh->phase_perc_s,
                        mesh->mu_s, mesh->T_s, mesh->p0_s, 0.0,
                        mesh->d0_s, mesh->phi0_s, mesh->X0_s,
                        mesh->sxxd0_s, mesh->szzd0_s, mesh->sxz0,
                        mesh->bet_s, mesh->div_u_s,
