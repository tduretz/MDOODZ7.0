...
- **`tensile_line_search = 1` can abort the process**: When the plasticity local-Newton line search fails to find a sufficient step (α < 0.01) within 100 iterations, `RheologyDensity.c:886` calls `LOG_WARN` followed by `exit(0)` — the test binary dies silently mid-run. Symptoms: GTest reports fewer results than expected, last visible log line is the WARN. For `plast = 2` (Popov 2025) tests with large per-step elastic overshoot (2·G·ε̇·dt ≫ yield), either (a) set `tensile_line_search = 0` and let Newton run undamped, (b) reduce `dt` so per-step overshoot is moderate, or (c) add a small `eta_vp` (≥ 1e16) for Perzyna damping. Rate-independent (`eta_vp = 0`) + paper-level strain rates works if line search is off.
- **CMake `file(COPY)` runs at configure time only**: `.txt` / `.c` fixture files under `TESTS/<Group>/` are copied into `cmake-build/TESTS/<Group>/` by the `file(COPY ... DESTINATION)` line in `TESTS/CMakeLists.txt` — but only when `cmake -B ...` is re-run, NOT when you edit the source file and rebuild. Symptoms: your .txt edits don't take effect; the test uses the stale build-tree copy. Fix: either `cp TESTS/<Group>/*.txt cmake-build/TESTS/<Group>/` manually after each edit, or re-run `cmake -B cmake-build` to refresh the copy.
- **`Model/Params[1]` is Lx, not step counter**: The BlankenBench-style extractor pattern (`readDoubleDataset(fd, "Model", "Params")`) exposes a `params[]` array, and `params[1]` is **xmax** (domain width), NOT the step number. Using it as step will produce nonsense filenames like `sII_step40000.dat` for step 40. Parse the step from the filename: `Output<NNNNN>.gzip.h5`. Layout: params[0]=time[s], params[1]=xmax, params[2]=zmax, params[3]=Nx, params[4]=Nz.
- **Step 0 contains HDF5 initialisation artefacts**: `/Centers/sxxd` and `/Centers/szzd` at `Output00000.gzip.h5` hold values ≈ 6×10¹⁴ Pa — the pre-integration elastic trial stress stored before any Stokes solve has run. `/Centers/P` is 0 (clean). When plotting trajectories or computing L2 errors against an analytical reference, clip or skip step 0 — otherwise a single 6e14 Pa outlier dominates the error norm. Test fixtures can safely evaluate assertions at the final step only.
- **MDOODZ writes no `/Centers/sII` field — reconstruct it**: MDLIB stores only deviatoric components (`sxxd`, `szzd` at centres; `sxz` at vertices). The stress second invariant must be reconstructed per cell: `sII = sqrt(0.5·(sxxd² + szzd²) + sxz_c²)` where `sxz_c` is averaged from the four surrounding vertices (`0.25·(sxz[j,i] + sxz[j,i+1] + sxz[j+1,i] + sxz[j+1,i+1])`). Same for strain-rate invariant `eII` from (`exxd`, `ezzd`, `exz`). See `TESTS/Popov2025Tests.cpp::reconstructSII()` / `reconstructSecondInvariant()` for a reusable helper.
- **Paper-parameter BC convention pitfall (factor-of-2)**: When a paper specifies `ε̇_xx = 2.333e-15 s⁻¹` and you implement it via custom outward BCs `Vx_W = -user2, Vx_E = +user2`, the imposed strain rate is `ε̇_xx = 2·user2 / Lx`, not `user2`. With `Lx = 1`: `user2 = ε̇_xx / 2 = 1.167e-15` to match the paper. Failing to halve doubles the imposed rate and yield onset occurs twice as fast as the paper shows.
- **`plast = 2` trial-pressure scheme — p* vs p_local**: The Popov 2025 combined tensile-cap yield surface uses a two-field trial-pressure formulation (paper §3.3). MDOODZ writes only the globally-discretised pressure `p*` to HDF5 as `/Centers/P` (= `mesh->p_in`). The yield-clamped local pressure `p_local = p* + K·θ̇_vp·dt` (paper Eq. 31) is **NOT exposed directly** — `mesh->p_corr` exists in MDLIB but stores something different (it gets reset to `P` on the final pass through `ViscosityConcise` because `F_trial ≤ 1e-17` after Stokes convergence, and is also used by `StokesAssemblyDecoupled.c` for a regularization term, not as the yield-clamped pressure).

  Reconstruct in postprocessing using `/Centers/divu_pl` (= θ̇_vp) and the simulation's K and dt:
  ```
  p_local = /Centers/P + K · dt · /Centers/divu_pl
  ```

  In the elastic regime `divu_pl = 0`, so `p_local = p*`. On the cap segment with `ψ > 0` they differ by ≈ 80 kPa for the paper's 0D parameters (≈ 8 % of τ_d). For yield-membership assertions and for the meridional `(P, τ_II)` trajectory plot in panel (d) of `fig5_0d_stress.gp`, use the reconstructed `p_local` — trajectory then sits cleanly on the yield envelope. Time-series plots of `P` (panels a–c) use `/Centers/P` as-is; the small cap-tip offset is visible as panel (a)'s saturation at ≈ -0.58 MPa vs `T_st = -0.5` MPa.

  The sign of the offset: Eq. 31 adds `+K·θ̇_vp·dt` to `p*` to get `p_local`. For extensional cap loading, `θ̇_vp > 0` (dilation), so `p_local > p*` (less tensile).

  **Lorenzo's BC observation**: in principle a Popov-style cap test could be designed so that `/Centers/P` itself is the yield-clamped pressure (no offset visible) — this requires driving the deformation by prescribing the plastic divergence rate (`divp`) at the boundaries rather than by an external velocity / strain rate. The current 0D fixtures use velocity BCs (eps_bg-style) so the offset is structural; reconstructing in postprocess is the pragmatic fix.
- **Two-tier assertion for plasticity tests**: For rate-law code-path validation (as done by `Popov2025Tests`), use (a) **final-state yield-surface membership**: reconstruct sII, assert `|sII - (k·P + c)|/(k·P + c) < 5 %` (DP) or `|sqrt(sII² + (P-py)²) - Ry|/Ry < 5 %` (cap), AND (b) **full-trajectory L2 against an independent re-implementation of the algorithm**: build a standalone scalar integrator (~200 lines) that does the same implicit Newton local solve, sample it at the same time steps, compare MDOODZ vs. helper via relative L2 in the per-step centre-cell trajectory. Sub-1 % L2 on the deviatoric component is a strong indicator of implementation correctness. See `TESTS/Popov2025/Popov0DAnalytical.cpp` (reference integrator) and `TESTS/Popov2025Tests.cpp::relativeL2()`.
- **`SetGridDensity` callback is a whole-simulation grid override, not a per-marker initial condition**: the `SetDensity` callback writes `particles->rho[np]` but this value is **overwritten** by grid-to-particle interpolation (`Main_DOODZ.c:369`), so it does not reach the body-force path. MDOODZ's body force (`EvaluateRHS` in `StokesRoutines.c`) uses `mesh->rho_n` / `mesh->rho_s`, which are computed from per-phase constants via `EvaluateDensity()`. For a benchmark that needs a spatially-varying density not expressible as per-phase constants (e.g., SolCx's `sin(π·z)·cos(π·x)`), use the new **`SetGridDensity`** callback — it overrides `mesh->rho_n` / `mesh->rho_s` at every rheology pass after the standard pipeline runs. Startup validation (in `Main_DOODZ.c`) **refuses** the combination of `SetGridDensity != NULL` with any non-zero `density_model` on any phase — "one source of truth per phase". Practical consequence: every `.txt` phase in a SolCx-style benchmark must declare `density_model = 0` explicitly (default is 3 per `ReadMatProps`). See `TESTS/SolCx/SolCx51.txt` for the reference configuration, and `TESTS/SolCxBenchmarkTests.cpp::SetGridDensity` for callback signature.
- **MDOODZ pressure output includes hydrostatic from initial lithostatic integration**: at simulation start, `ComputeLithostaticPressure` populates `mesh->p_in` with the integrated `∫ρ·g dz` (see `Main_DOODZ.c:358-362`). After the Stokes solve, `Centers/P` written to HDF5 is the **total** pressure (dynamic + lithostatic baseline), not the dynamic perturbation. To compare against benchmarks that produce dynamic pressure only (e.g., Velic analytical), enable `track_T_P_x_z = 1` in the `.txt` and read `Centers/P0` (initial lithostat) — the dynamic component is `P - P0`. For SolCx this is handled by using L1(P) on the raw field with a threshold tolerant of the lithostat's spatial structure; for benchmarks where dynamic-only comparison matters, subtract P0 explicitly.
...
| `SolCxBenchmarkTests.cpp` | AnalyticalSelfConsistency | Velic analytical port evaluates cleanly (no NaN/Inf, divergence-free) at 5 sample points |
| `SolCxBenchmarkTests.cpp` | L2Error | Step-η Stokes at 51×51: L2(Vx)<0.25, L2(Vz)<0.25, L1(P)<1e-2 (measured 7.4e-2 / 8.5e-2 / 2.3e-3) |
| `SolCxBenchmarkTests.cpp` | GridConvergence | Convergence orders (41→81): Vx ≥ 0.7, Vz ≥ 0.7, L1(P) ≥ 0.5 (measured 0.98 / 0.99 / 2.57) |
