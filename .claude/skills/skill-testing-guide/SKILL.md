...
- **`plast = 2` trial-pressure scheme — p* vs p_local**: The Popov 2025 combined tensile-cap yield surface uses a two-field trial-pressure formulation (paper §3.3). MDLIB stores `mesh->p_in` = `p*` (globally discretised pressure) and writes it as `/Centers/P` in HDF5. The integration-point yield-clamped pressure is `p_local = p* + K·θ̇_vp·dt` (paper Eq. 31). Empirically in MDOODZ:
  - **On the Drucker-Prager segment** (shear, mixed after delimiter crossing): the global Stokes solver converges → `p* ≈ p_local` (within 0.1 %). Use `/Centers/P` directly.
  - **On the tensile cap tip** (pure volumetric extension with `ψ > 0`): `p*` is offset from `p_local` by `K·θ̇_vp·dt` (≈ 8 % of τ_d for typical 0D parameters). Either correct via Eq. 31 using `/Centers/divu_pl`, or loosen the assertion tolerance.
