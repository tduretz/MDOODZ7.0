...
%% Tracer index from reduced size array
red_ind  = 60000; 
...
for istep=istart:ijump:iend
    ...
    if particles
        ...
        Part.ind   = hdf5read(filename_p,'/Particles/index');
        
        %% Determine initial tracer global index
        if istep == istart
            glob_ind = Part.ind(red_ind);
        end
        ...
        %% Plot tracer using reduced index
        plot(Part.x(ind), Part.z(ind), 'w*')
...
%            end
        ...
        if printfig == 1
            if crop == 1
                print([path, './Fig_Phases/Crop_PhasesPart', num2str(istep,'%05d'),file_suffix], format, res)
            else
                print([path, './Fig_Phases/Fig_PhasesPart', num2str(istep,'%05d'),file_suffix], format, res)
            end
            close all
        end
