...
%         z_tab(:,:) = z_arr;
        ...
        %--------------------------------------------------
        % plot pure_simple_shear components
        %--------------------------------------------------
        if ( pure_simple_shear == 1 )
            ...
            x0    = hdf5read(filename,'/Centers/x0'); x0   = cast(x0  , 'double');
            z0    = hdf5read(filename,'/Centers/z0'); z0   = cast(z0  , 'double');
            ...
            subplot (211)
            imagesc(xc_plot, zc_plot, x0/1000 );
            ...
            caxis([min(xc_plot) max(xc_plot)])

            subplot (212)
            imagesc(xc_plot, zc_plot, z0/1000 );
            shading flat, axis xy image, colorbar;
            ...
            PSuc    = zeros(size(x0,2),1);
            PSlc    = zeros(size(x0,2),1);
            PSm     = zeros(size(x0,2),1);
            
            SSuc    = zeros(size(x0,2),1);
            SSlc    = zeros(size(x0,2),1);
            SSm     = zeros(size(x0,2),1);
            
            for ix = 1:size(VizGrid.ph,2)
                
                ibot    = find(VizGrid.ph(:,ix)==2 | VizGrid.ph(:,ix)==(2+4), 1, 'first'  );
...
%                 SSlc(ix) = (x0(itop,ix)-x0(ibot,ix))/dH;
            ...
            subplot(211), hold on 
            plot(VizGrid.x_plot, 1-1./PSuc)
            plot(VizGrid.x_plot, 1-1./PSlc)
            plot(VizGrid.x_plot, 1-1./PSm)
        ...
        if (thinning==1)
            ...
            Huc = zeros(size(VizGrid.ph_hr,2),1);
            Hlc = zeros(size(VizGrid.ph_hr,2),1);
            Hml = zeros(size(VizGrid.ph_hr,2),1);
            
            for ix = 1:size(VizGrid.ph_hr,2)
                
                iml_bot    = find(VizGrid.ph_hr(:,ix)==2 | VizGrid.ph_hr(:,ix)==(2+4), 1, 'first'  );
                iml_top    = find(VizGrid.ph_hr(:,ix)==2 | VizGrid.ph_hr(:,ix)==(2+4), 1, 'last'  );
                ilc_bot    = find(VizGrid.ph_hr(:,ix)==1 | VizGrid.ph_hr(:,ix)==(1+4), 1, 'first'  );
                ilc_top    = find(VizGrid.ph_hr(:,ix)==1 | VizGrid.ph_hr(:,ix)==(1+4), 1, 'last'  );
                iuc_bot    = find(VizGrid.ph_hr(:,ix)==0 | VizGrid.ph_hr(:,ix)==(0+4), 1, 'first'  );
                iuc_top    = find(VizGrid.ph_hr(:,ix)==0 | VizGrid.ph_hr(:,ix)==(0+4), 1, 'last'  );
                Huc(ix)    = (iuc_top-iuc_bot-1)*dz/2/1e3;
