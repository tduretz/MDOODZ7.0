...
path = '/Users/imac/REPO_GIT/MDOODZ6.0/SOURCE/Huismans_Reg8e19_NoIts_030621/';
path = '/Users/imac/REPO_GIT/MDOODZ6.0/SOURCE/Cindy01/'
...
%             strmin = min(log10(eII(:)));
%             strmax = max(log10(eII(:)))/4;
...
%             end
        ...
        if(grain_size==1)
            ...
            colormap('jet')
            imagesc(xc_plot, zc_plot, log10(d)), axis xy;
            shading flat% caxis ([-1 size(map,1)-2])%, 
            axis image
            %         colorbar('Location', 'SouthOutside')
