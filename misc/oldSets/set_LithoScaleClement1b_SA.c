...
//        if (particles->z[np] < -5e3/scaling.L && particles->x[np] > -18e3/scaling.L && particles->x[np] < 18e3/scaling.L)  particles->phase[np] = 0;
    ...
    if( particles->z[np] > TopoLevel * exp(-particles->x[np]*particles->x[np]/sig/sig))  particles->phase[np] = 0;
...
//
    double V_W = -model->xmin*model->bkg_strain_rate;
    double V_E = -model->xmax*model->bkg_strain_rate;
    ...
    for (l=0; l<mesh->Nz; l++) {
        for (k=0; k<mesh->Nx+1; k++) {
            ...
            if ( mesh->BCv.type[c] != 30 ) {
                ...
                if (l==0 ) {
                    mesh->BCv.type[c] = 0;
                    mesh->BCv.val[c]  = 0*mesh->zg_coord[l] * model->bkg_strain_rate;
                }
                
                // Matching BC nodes NORTH
                if (l==mesh->Nz-1 ) {
                    mesh->BCv.type[c] = 0;
                    mesh->BCv.val[c]  = (mesh->zg_coord[l]-model->zmin) * model->bkg_strain_rate;
                }
