...
//        }
        ...
        double X,Xn,Z,Zn, xc=0.0, zc=model.zmin, la= 1.00*rad, sa = 1.00*rad, theta=(30.0)*M_PI/180.0;
        xc=0.0;
        zc=model.zmin;
			...
			for (k=0; k<mesh->Nx; k++) {
                ...
                if ( mesh->BCu.type[c] != 30 ) {
                    ...
                    if (model->shear_style== 1 ) {
                        ...
                        if (l==0 ) { //&& (k>0 && k<NX-1) ) {
                            mesh->BCu.type[c] =  11;
//                            mesh->BCu.val[c]  = -model->bkg_strain_rate*Lz/1.0;
                            mesh->BCu.val[c]  = 2.0*(mesh->zvx_coord[l]-model->zmin)*model->bkg_strain_rate;
                        }
