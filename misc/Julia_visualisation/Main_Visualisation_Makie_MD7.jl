...
using HDF5, GLMakie, Printf, Colors, ColorSchemes
My = 1e6*365*24*3600
...
function main()
    ...
    for istep=file_start:file_step:file_end
        ...
        f = Figure(resolution = ( Lx/Lz*600,600), fontsize=25, aspect = 2.0)
        ...
        if field==:phases
            ...
            if printfig @printf("Write %s\n", path*"Phase"*@sprintf("%05d", istep)*".png")
                save(path*"Phase"*@sprintf("%05d", istep)*".png",  f, px_per_unit = 4) end
        end
