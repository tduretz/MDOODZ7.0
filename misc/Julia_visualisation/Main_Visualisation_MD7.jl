...
function main()
    ...
    for istep=file_start:file_step:file_end
        ...
        if field==:phases      p = heatmap!(xc_ph./1e3, zc_ph./1e3, ph', c=:turbo)   end
        if field==:density     p = heatmap!(xc./1e3, zc./1e3, (ρc'), c=:turbo) end
        if field==:viscosity   p = heatmap!(xc./1e3, zc./1e3, log10.(ηc'), c=:turbo) end
        if field==:pl_srate    p = heatmap!(xc./1e3, zc./1e3, log10.(ε̇pl'), c=:turbo) end
        if field==:pressure    p = heatmap!(xc./1e3, zc./1e3, (P'), c=:turbo) end
        if field==:grain_size  p = heatmap!(xc./1e3, zc./1e3, log10.(d'), c=:turbo) end
        if field==:velocity_x  p = heatmap!(xc./1e3, zc./1e3, Vxc', c=:turbo) end
        if field==:velocity_z  p = heatmap!(xc./1e3, zc./1e3, Vzc', c=:turbo) end
