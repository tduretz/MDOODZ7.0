...
function main_simple_ani_vis()
    ...
    ε̇yyd       = -ε̇xxd
    ε̇xyd       = 0*(1.0 - pure_shear)*5.0 +0* 5.0/3
    ...
    p2 = plot(title="Ellipse", xlabel="τxx", ylabel="τxy")
    α  = 0.:0.01:2π
    xc = 2*cos.(α)
    yc = 2*sin.(α)
    p2 = plot(xc, yc, label="isotropic, δ = 1")
    xc = 2*cos.(α)
    yc = 2/ani_fac_v*sin.(α)
    p2 = plot!(xc, yc, label="anisotropic, δ = 3")
    p2 = scatter!(τxx_rot, τxy_rot, label="model", aspect_ratio=1.0)
