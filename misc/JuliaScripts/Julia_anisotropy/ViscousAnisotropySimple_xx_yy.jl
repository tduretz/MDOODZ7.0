...
function main_simple_ani_vis()
    ...
    ε̇yyd       = -0*ε̇xxd
    ε̇xyd       = (1.0 - pure_shear)*5.0 + 0*5.0/3
    ε̇bg        = sqrt(ε̇xxd^2 + ε̇xyd^2)
    # Arrays
    θ          = LinRange( 0.0, π, 51 ) .- 0π/2
    τii_cart   = zero(θ)
    τii_rot1   = zero(θ)
    τii_rot2   = zero(θ)
    τii_rot3   = zero(θ)
    ...
    for i in eachindex(θ)
        ...
        D           = 2*ηv*[1 0 0; 0 1/ani_fac_yy 0; 0 0 1.0/ani_fac_xy;]
        τ           = D*ε
        Y2          = 0.5*τ[1]^2 + 0.5*ani_fac_yy^2*τ[2]^2 + τ[3]^2*ani_fac_xy^2
        τii_rot1[i] = sqrt(Y2)
        ...
        # if one does the same by scaling the strain rate invariant by ani_fac_xy...
        I2          = 0.5*ε[1]^2 + 0.5*ε[2]^2/ani_fac_yy^2 + ε[3]^2/ani_fac_xy^2
