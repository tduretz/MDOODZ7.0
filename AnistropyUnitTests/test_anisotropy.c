...
void StressEffectiveViscosity_TensorInterface( Tensor2D* Tau, Tensor2D* Eps, mat_params* materials ) {
    ...
    const double eta_v = materials->eta_v;
    // Compute isotropic stress
    Tau->xx = 2*eta_v*Eps->xx;
    Tau->zz = 2*eta_v*Eps->zz;
    Tau->xz = 2*eta_v*Eps->xz;
}
...
//       // ------------------------------------- TEST 1 ------------------------------------- //
//     printf("No tensor interface on %02d threads\n", omp_get_num_threads() );
//     t_mean=0.;
...
//     // ------------------------------------- TEST 2 ------------------------------------- //
//     printf("With tensor interface on %02d threads\n", omp_get_num_threads() );       
//     t_mean=0.;
...
//         // Compute rheology at all points
// #pragma omp parallel for private(Tau, Eps) shared(mesh) firstprivate(materials) 
//         for (int k=0; k<Nx*Nz; k++) {
//             Eps.xx = mesh.Exx[k];
//             Eps.zz = mesh.Ezz[k];
//             Eps.xz = mesh.Exz[k];
